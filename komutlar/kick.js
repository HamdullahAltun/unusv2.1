...
module.exports.run = async (bot, message, args) => {
  ...
  const u = (message.mentions.users.first() || message.guild.members.get(args[0]))
  if (!u) {
    return message.channel.send("Lütfen Sunucudan Atılacak Kişiyi Belirtiniz!"
    );
  }
 ...
 if (!reason) reason = 'Belirtilmemiş.'
  ...
  message.channel.send(embed).then(async function(sentEmbed) {
   ...
   await u.send(`**${message.guild.name}** Sunucusundan **${message.author.tag} - ${message.author.id}** Yetkilisi tarafından ${reason} Sebebiyle **Atıldınız.**`)
   await sleep(1000)
     ...
     message.guild.member(u).kick(`Sebep: ${reason} | Yetkili: ${message.author.tag} - ${message.author.id}`);
      }
